...
async function example3_RichContent() {
  ...
  const description = textToADF(
    "The API is returning unexpected errors when processing user requests.",
  );
...
function example5_EnvironmentCheck() {
  ...
  const requiredVars = ["JIRA_USER_EMAIL", "JIRA_API_TOKEN", "JIRA_BASE_URL", "JIRA_PROJECT_KEY"];

  // No optional variables needed anymore - GitHub URL goes in description
  ...
  console.log("  ✅ GitHub URL is automatically included in the description");
...
function example6_CLIUsage() {
  ...
  const examples = [
    ...
    },
    {
      ...
      command:
        'node create.js "Task" "[GitHub] Fix mobile bug" "Description here" "https://github.com/user/repo/issues/1" "username" "2023-01-01T00:00:00Z"',
    },
    {
      name: "Story with multiline description",
      command:
        'node create.js "Story" "User dashboard" "Create a user dashboard\\nwith analytics\\nand user data"',
    },
  ];
  ...
  console.log("\n  ✅ No custom field setup required - GitHub URL goes in description!");
}
